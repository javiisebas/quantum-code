...
import type { WerewolfRoom } from './domain';
...
export function WerewolfPlayer({ code }: { code: number | null }) {
    return (
        <PlayerShell<WerewolfRoom> game={werewolfManifest.id} gameName={werewolfManifest.name} code={code}>
            {(payload, seat) => <WerewolfCard payload={payload} seat={seat} />}
        </PlayerShell>
    );
}
