 ...
 * (which encodes the full `/join/codenames?code=…` URL) or by typing the code, so
...
export const ModalCodeGameContent: FC<ModalCodeGameContentProps> = ({ code }) => {
    ...
    const joinUrl = useMemo(
        () => (origin ? `${origin}/join/codenames?code=${code}` : ''),
        [origin, code],
    );
        ...
        <div>
            ...
            <ModalBody className="items-center gap-5 pb-8">
                <p className="text-center text-sm text-gray-300">
                    Los espías escanean el QR o entran en{' '}
                    <span className="font-semibold">/join</span> e introducen el código.
                </p>
